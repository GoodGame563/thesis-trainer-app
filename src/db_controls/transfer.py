...
async def create_transfer(player_id: int, team_id: int, transfer_date: date) -> Transfer:
    async with db_connect() as db:
        db.row_factory = aiosqlite.Row
        ...
        team = Team(
            id=team_row["id"],
            name=team_row["name"],
            path_to_logo=team_row["logo"]
        )
        
        async with db.execute("""
            INSERT INTO transfers (player_id, team_id, date)
            VALUES (?, ?, ?)
        """, (player_id, team_id, transfer_date.isoformat())) as cursor:
            await db.commit()
