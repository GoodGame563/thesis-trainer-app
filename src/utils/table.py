...
def button_func(input_func):
    def output_func(*args):
        args[1].control.disabled = True
        input_func(*args)
        args[1].control.disabled = False
...
class InformationTable(Card):
    ...
    def __init__(self, open_user_view):
        self.rows = []
        self.columns = []
    ...
    async def open_user(self, e):
        await self.open_user_view.open_user(e.control.key)
